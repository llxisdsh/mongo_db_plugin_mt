...
mongo_db_plugin_impl::~mongo_db_plugin_impl() {
   if (!startup) {
      try {
         ...
         for(size_t i=0;i<consume_thread.size();++i)
         {
...
void mongo_db_plugin_impl::init() {
   ...
   {
      consume_thread.push_back(boost::thread([this] { consume_blocks(); }));
   }
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("mongodb-thread-size", bpo::value<uint32_t>()->default_value(1),
         "The number of MongoDB plugin threads, This value is less than the maxPoolSize parameter of mongodb-uri")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               ...
               " Example: mongodb://127.0.0.1:27017/EOS?maxPoolSize=100")
...
void mongo_db_plugin::plugin_initialize(const variables_map& options)
{
   try {
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.count( "mongodb-thread-size" )) {
            my->max_thread_size = options.at( "mongodb-thread-size" ).as<uint32_t>();
         }
